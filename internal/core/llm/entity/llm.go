...
func NewBaseLLM(model llms.Model, features []ModelFeature, metadata map[string]any) *BaseLLM {
	return &BaseLLM{
		Model:    model,
		features: features,
		metadata: metadata,
...
type ModelFactory func(modelName string, config map[string]any) (BaseLanguageModel, error)
