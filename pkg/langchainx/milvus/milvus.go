...
func (s Store) DeleteDocument(ctx context.Context, key string, value any) error {
	expr := fmt.Sprintf("%s == %s", key, value)

	return s.client.Delete(ctx, s.collectionName, s.partitionName, expr)
}

func (s Store) UpdateDocument(ctx context.Context, column ...entity.Column) error {
	_, err := s.client.Upsert(ctx, s.collectionName, s.partitionName, column...)
	return err
}
