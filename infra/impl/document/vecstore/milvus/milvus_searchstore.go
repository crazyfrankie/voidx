...
import (
	...
	"github.com/crazyfrankie/voidx/infra/contract/document"
...
func (m *milvusSearchStore) Store(ctx context.Context, docs []*schema.Document, opts ...indexer.Option) (ids []string, err error) {
	...
	implSpecOptions := indexer.GetImplSpecificOptions(&vecstore.IndexerOptions{}, opts...)
	defer func() {
		if err != nil {
...
func (m *milvusSearchStore) Retrieve(ctx context.Context, query string, opts ...retriever.Option) ([]*schema.Document, error) {
	...
	implSpecOptions := retriever.GetImplSpecificOptions(&vecstore.RetrieverOptions{}, opts...)
...
func (m *milvusSearchStore) Delete(ctx context.Context, ids []string) error {
	...
	_, err := m.config.Client.Delete(ctx,
		client.NewDeleteOption(m.collectionName).WithInt64IDs(vecstore.FieldID, int64IDs))
...
func (m *milvusSearchStore) documents2Columns(ctx context.Context, docs []*schema.Document, indexingFields sets.Set[string]) (
	...
	colTypeMapping := map[string]vecstore.FieldType{
		vecstore.FieldID:          vecstore.FieldTypeInt64,
		vecstore.FieldCreatorID:   vecstore.FieldTypeInt64,
		vecstore.FieldTextContent: vecstore.FieldTypeText,
	}
	for _, doc := range docs {
		...
		for field := range ext {
			...
			case uint, uint8, uint16, uint32, uint64, uintptr:
				var c []int64
				if container == nil {
					colTypeMapping[field] = vecstore.FieldTypeInt64
				} else {
			...
			case string:
				var c []string
				if container == nil {
					colTypeMapping[field] = vecstore.FieldTypeText
				} else {
			...
			case []float64:
				...
				if container == nil {
					container = c
					colTypeMapping[field] = vecstore.FieldTypeDenseVector
				} else {
			...
			case map[int]float64:
				...
				if container == nil {
					container = c
					colTypeMapping[field] = vecstore.FieldTypeSparseVector
				} else {
	...
	colMapping[vecstore.FieldID] = ids
	colMapping[vecstore.FieldCreatorID] = creatorIDs
	colMapping[vecstore.FieldTextContent] = contents
	...
	for fieldName, container := range colMapping {
		...
		case vecstore.FieldTypeText:
			...
			if _, indexing := indexingFields[fieldName]; indexing {
				if fieldName == vecstore.FieldTextContent && !emptyContents {
	...
	for _, r := range result {
		for i := 0; i < r.ResultCount; i++ {
			...
			for _, field := range r.Fields {
				...
				case vecstore.FieldTextContent:
...
func (m *milvusSearchStore) dsl2Expr(src map[string]interface{}) (string, error) {
	...
	dsl, err := vecstore.LoadDSL(src)
	if err != nil {
		return "", err
	}

	var travDSL func(dsl *vecstore.DSL) (string, error)
	travDSL = func(dsl *vecstore.DSL) (string, error) {
		...
		case vecstore.OpAnd, vecstore.OpOr:
			sub, ok := dsl.Value.([]*vecstore.DSL)
			if !ok {
